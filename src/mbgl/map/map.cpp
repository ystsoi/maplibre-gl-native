...
LatLngBounds Map::latLngBoundsForCameraUnwrapped(const CameraOptions& camera) const {
    ...
    auto bounds = impl->unwrappedLatLngBounds(shallow);
    return bounds;
}

LatLngBoundsZoom Map::latLngBoundsZoomForCameraUnwrapped(const CameraOptions& camera) const {
    Transform shallow{impl->transform.getState()};

    shallow.jumpTo(camera);

    auto bounds = impl->unwrappedLatLngBounds(shallow);
    double zoom = shallow.getZoom();

    auto retVal = LatLngBoundsZoom(bounds, zoom);
    return retVal;
}
